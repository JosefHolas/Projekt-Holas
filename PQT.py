...
# Kontrola, zda tabulka má sloupec 'name'. Pokud ne, přidáme ho.
def check_column_exists():
    c.execute("PRAGMA table_info(scores);")
    columns = [column[1] for column in c.fetchall()]
    if 'name' not in columns:
        c.execute("ALTER TABLE scores ADD COLUMN name TEXT;")
        conn.commit()

# Volání funkce pro kontrolu sloupce při každém spuštění
check_column_exists()
...
def game_over(name):
    ...
    game_window.blit(game_over_surface, (window_x / 4, window_y / 6))
